...
static term binary_to_atom(Context *ctx, int argc, term argv[], int create_new)
{
    ...
    if (!encode_latin1_to_utf8) {
        ...
        size_t encoded_len = atom_string_len * 2;
        if (encoded_len > 255) {
            RAISE_ERROR(SYSTEM_LIMIT_ATOM);
        }
        atom = malloc(encoded_len + 1);
        ((uint8_t *) atom)[0] = encoded_len;
        uint8_t *atom_data = ((uint8_t *) atom) + 1;
        for (size_t i = 0; i < atom_string_len; i++) {
            size_t out_size;
            bitstring_utf8_encode(((uint8_t) atom_string[i]), &atom_data[i * 2], &out_size);
            if (UNLIKELY(out_size != 2)) {
                // unreachable, but let's detect implementation bugs
                abort();
            }
        }
    }
...
static term nif_erlang_atom_to_binary_2(Context *ctx, int argc, term argv[])
{
    ...
    if (!encode_to_latin1) {
        ...
        char *utf8_tmp_buf = malloc(atom_len);
        if (IS_NULL_PTR(utf8_tmp_buf)) {
            RAISE_ERROR(OUT_OF_MEMORY_ATOM);
        }
        atom_table_write_bytes(glb->atom_table, atom_ref, atom_len, utf8_tmp_buf);
        size_t encoded_len = atom_len / 2;
        ...
        for (size_t i = 0; i < encoded_len; i++) {
            ...
            uint32_t codepoint;
            if (UNLIKELY(bitstring_utf8_decode(
                             (uint8_t *) &utf8_tmp_buf[i * 2], 2, &codepoint, &codepoint_size)
                        != UnicodeTransformDecodeSuccess
                    || (codepoint > 255))) {
                free(utf8_tmp_buf);
                RAISE_ERROR(BADARG_ATOM);
...
static term nif_erlang_atom_to_list_1(Context *ctx, int argc, term argv[])
{
    ...
    bool latin1 = atom_len == u8len;

    size_t list_len = latin1 ? atom_len : u8len;

    if (UNLIKELY(memory_ensure_free_opt(ctx, list_len * 2, MEMORY_CAN_SHRINK) != MEMORY_GC_OK)) {
        free(atom_buf);
        RAISE_ERROR(OUT_OF_MEMORY_ATOM);
    }
